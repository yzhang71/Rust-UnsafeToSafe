...
fn main() {
    ...
    dst[2..2 + cnt].copy_from_slice(&src[2..2 + cnt]);
...
fn main() {
    ...
    index = unsafe {vec.get_unchecked(5)};    
    print!("Index: {:?} \n", index);
}
...
fn main() {
    ...
    string = unsafe {std::str::from_utf8_unchecked_mut(&mut sparkle_heart)};
...
fn main() {
    ...
    unsafe {
        char_valid = std::char::from_u32_unchecked(0x2764);
